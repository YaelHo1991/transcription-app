...
interface AutoDetectMessage {
  type: 'START_TYPING' | 'STOP_TYPING' | 'ENABLE' | 'DISABLE' | 'SET_MODE' | 'UPDATE_SETTINGS' | 'ACTION' | 'STATUS';
  mode?: 'regular' | 'enhanced';
  settings?: AutoDetectSettings;
  action?: 'pause' | 'resume';
}

interface AutoDetectSettings {
  enabled: boolean;
  mode: 'regular' | 'enhanced';
  firstPauseDelay: number;
  secondPauseDelay: number;
...
let enhancedState: 'playing' | 'first_pause' | 'typing_after_pause' | 'waiting_for_second_pause' = 'playing';
...
function sendAction(action: 'pause' | 'resume') {
  self.postMessage({
    type: 'ACTION',
    data: {
      action,
...
self.onmessage = (event: MessageEvent<AutoDetectMessage>) => {
