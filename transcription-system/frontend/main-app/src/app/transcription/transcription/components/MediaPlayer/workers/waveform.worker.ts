...
interface WaveformMessage {
  type: 'ANALYZE' | 'CANCEL' | 'PROGRESS' | 'COMPLETE' | 'ERROR';
...
async function analyzeAudioBuffer(audioBuffer: ArrayBuffer, sampleRate: number) {
  try {
    isProcessing = true;
    shouldCancel = false;
...
// Multi-resolution waveform generation for zoom support
async function generateMultiResolutionWaveform(audioBuffer: ArrayBuffer, sampleRate: number) {
  const resolutions = [100, 500, 1000, 2000, 5000]; // Different zoom levels
  ...
  try {
    ...
    for (const targetPoints of resolutions) {
      ...
      // Generate waveform at this resolution
      for (let i = 0; i < actualPoints; i++) {
        const start = i * samplesPerPoint;
        const end = Math.min(start + samplesPerPoint, channelData.length);
      ...
      waveforms.set(targetPoints, peaks);
    }
